...
class UserCrudCustomerPermissions(BasePermission):
...
class UserCrudSubscriptionPermissions(BasePermission):
        
    def has_object_permission(self, request:HttpRequest, view, obj:Subscription):
        user = get_user_from_request(request)
        if request.method == "GET" : # see obj
            return self.check_fetch(user,obj)
        elif request.method == "POST" : # create new
    ...
    def check_fetch(self, user:BaseUser , obj:Subscription): 
        if self.is_user_manager(user):
            return True
        elif user.role == user.Role.SALES or user.role == user.Role.CS:
    ...
    def check_update(self, user:BaseUser , obj:Subscription ):
        if self.is_user_manager(user):
            return True
        elif user.role == user.Role.SALES or user.role == user.Role.CS:
            return (user == obj.creator) or ( user == obj.cs )
